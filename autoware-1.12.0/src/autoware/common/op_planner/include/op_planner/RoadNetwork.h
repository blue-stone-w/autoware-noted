...
class WayPoint
{
 public:
  ...
  Rotation rot;
  double v;
  double cost; // length of path from some point to this point
  double timeCost;
  double totalReward;
...
// Extract close lanes to construct local trajectory/lanes for cur pose.
// This class is info between cur pose and local lanes.
class RelativeInfo
{
  // p0p1 closest points in lanes; p0 is last point, pi is next point.
 public:
  double perp_distance;      // Y axis of car is front; distance between car and lanes on Y-axis
  double to_front_distance;  // negative; distance between car and next point in lanes on X-axis
  double from_back_distance; // distance between perpendicular point and p0
  int iFront;                // index of next point in trajectory
  int iBack;                 // index of last point in trajectory
  int iGlobalPath;           // ???
  WayPoint perp_point;       // perpendicular;interception point between Y-axis and line p0p1
...
// obj during perception and track
class DetectedObject
{
 public:
  ...
  std::vector<WayPoint> centers_list; // store all poses of center everytime detected.
  std::vector<GPSPoint> contour;      // vertex of polygon
  std::vector<std::vector<WayPoint>> predTrajectories;
  std::vector<WayPoint *> pClosestWaypoints;
  double w;
  double l;
  double h;
  double distance_to_center; // used to compute in which interest circle this obj locates.

  double actual_speed;
  double actual_yaw;

  bool bDirection; // whether direction valid
  bool bVelocity;  // whether velcolity valid
  int acceleration;

  int acceleration_desc;   // direction: positive, negative, zero(when closr to 0)
  double acceleration_raw; // original value, which will be processed to get final acc

  LIGHT_INDICATOR indicator_state;

  int originalID; // ID of corresponding cluster
