...
bool PlanningHelpers::GetRelativeInfoLimited(const std::vector<WayPoint> &trajectory, const WayPoint &p, RelativeInfo &info, const int &prevIndex)
{
  ...
  WayPoint p0, p1; // two closest points in trajectory for cur pose
  ...
  if (trajectory.size() == 2)
  {
    ...
    if (info.iFront == 0) // didn't find closest point in trajectory
    {
      p0 = _trajectory.at(info.iFront);
      p1 = _trajectory.at(info.iFront + 1);
    }
    else if (info.iFront > 0 && info.iFront < _trajectory.size() - 1) // found closest point in trajectory
    {
      p0 = _trajectory.at(info.iFront - 1);
      p1 = _trajectory.at(info.iFront);
    }
    else // closest point is last point
    {
      p0 = _trajectory.at(info.iFront - 1);
    ...
    WayPoint prevWP = p0; // previous
    Mat3 rotationMat(-p1.pos.a);
    Mat3 translationMat(-p.pos.x, -p.pos.y);
    Mat3 invRotationMat(p1.pos.a);
    Mat3 invTranslationMat(p.pos.x, p.pos.y);

    p0.pos = translationMat * p0.pos; // relative translation from p to p0
    p0.pos = rotationMat * p0.pos;    // relative rotation from p1 to p0

    p1.pos = translationMat * p1.pos; // relative translation from p to p1
    p1.pos = rotationMat * p1.pos;    // relative rotation from p0 to p0

    double m           = (p1.pos.y - p0.pos.y) / (p1.pos.x - p0.pos.x);
    info.perp_distance = p1.pos.y - m * p1.pos.x; // solve for x = 0; interception with axis Y
    ...
    info.perp_point       = p1;
    info.perp_point.pos.x = 0;                  // on the same y axis of the car
    info.perp_point.pos.y = info.perp_distance; // perpendicular distance between the car and the _trajectory

    info.perp_point.pos = invRotationMat * info.perp_point.pos;    // rotation of p1
    info.perp_point.pos = invTranslationMat * info.perp_point.pos; // global pose
    ...
    else if (info.iFront == _trajectory.size() - 1)
    {
      ...
      double angle_befor_last = UtilityH::FixNegativeAngle(atan2(_trajectory.at(s - 2).pos.y - _trajectory.at(s - 1).pos.y,
                                                                 _trajectory.at(s - 2).pos.x - _trajectory.at(s - 1).pos.x));
      double angle_from_perp  = UtilityH::FixNegativeAngle(atan2(info.perp_point.pos.y - _trajectory.at(s - 1).pos.y,
...
// find closest point in tractory for p and return index of closest point
int PlanningHelpers::GetClosestNextPointIndexFast(const vector<WayPoint> &trajectory, const WayPoint &p, const int &prevIndex)
{
  ...
  // find closest points in trajectory, last point excluded.
  for (int i = 0; i < size; i += skip)
  {
    if (i + skip / 2 < size)
      d = (distance2pointsSqr(trajectory[i].pos, p.pos) + distance2pointsSqr(trajectory[i + skip / 2].pos, p.pos)) / 2.0;
    else
      d = distance2pointsSqr(trajectory[i].pos, p.pos); // square of distance
    if (d < minD)
    {
      iStart    = i - skip;
      iEnd      = i + skip;
  ...
  // check last point in trajectory
  if ((size - skip / 2 - 1) > 0)
  ...
  if (min_index < size - 1) // if find a point
  {
    GPSPoint curr, next;
    curr = trajectory[min_index].pos;
    next = trajectory[min_index + 1].pos;
...
void PlanningHelpers::SmoothPath(vector<WayPoint> &path, double weight_data, double weight_smooth, double tolerance)
{
  if (path.size() <= 2)
  {
