                                                ...
                                                const sensor_msgs::PointCloud2::ConstPtr &in_child_cloud_msg)
{
  ...
  // the transformation epsilon in order for an optimization to be considered as having converged to the final solution.
	...
	ndt.setStepSize(ndt_step_size_); // Set/change the newton line search maximum step length.
	ndt.setResolution(ndt_resolution_); // Set/change the voxel grid resolution.
  ...
  // meet this condition only when first iteration
