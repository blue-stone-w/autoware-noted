...

{
  ...
  std::vector<cv::Point2f> clicked_image_points_;    // store points, not images
  std::vector<cv::Point3f> clicked_velodyne_points_; // store points, not clouds


  bool is_rotation_matrix(const cv::Mat &in_matrix)
  {
    cv::Mat rotation_matrix;
    transpose(in_matrix, rotation_matrix); // I = rotation.inverse * rotation.transpose
  ...
  cv::Point3f get_rpy_from_matrix(const cv::Mat &in_rotation)
  {
    ...
    float sy = sqrt(in_rotation.at<double>(0, 0) * in_rotation.at<double>(0, 0) + in_rotation.at<double>(1, 0) * in_rotation.at<double>(1, 0));
  ...
  void CalibrateSensors()
  {
    ...
    if (clicked_velodyne_points_.size() == clicked_image_points_.size() && clicked_image_points_.size() > 8)
    {
      ...
      std::cout << "RPY: " << get_rpy_from_matrix(camera_velodyne_rotation) << std::endl
                << std::endl;
  ...
  {
    clicked_image_points_.push_back(cv::Point2f(in_clicked_point.point.x, in_clicked_point.point.y));
  ...
  {
    // if param haven't been initialized
    if (camera_instrinsics_.empty() || distortion_coefficients_.empty())
    {
