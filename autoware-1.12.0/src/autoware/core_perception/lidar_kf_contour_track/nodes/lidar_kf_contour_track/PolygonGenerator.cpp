...
namespace ContourTrackerNS
{
			...
			const double &polygon_resolution)
	{
		...
		for (unsigned int i = 0; i < cluster.points.size(); i++)
		{
			...
			p.pos.z = original_centroid.z; // unused

			PlannerHNS::GPSPoint v(p.pos.x - original_centroid.x, p.pos.y - original_centroid.y, 0, 0); // x,y,z,direction
			p.cost = pointNorm(v);																																			// length
			p.pos.a = UtilityHNS::UtilityH::FixNegativeAngle(atan2(v.y, v.x)) * (180. / M_PI);					// ensure 0<=a<=2PI

			for (unsigned int j = 0; j < m_Quarters.size(); j++)
			{
				if (m_Quarters.at(j).UpdateQuarterView(p)) // put p into corresponding quater
					break;
			}
		}
		m_Polygon.clear();
		...
		//	//Fix Resolution: limit length of poligon's sides
		bool bChange = true;
		while (bChange && m_Polygon.size() > 1)
		{
			...
			for (unsigned int i = 0; i < m_Polygon.size(); i++)
			{
				...
				double d = hypot(p2.y - p1.y, p2.x - p1.x); // distance p1p2 (length of side)

				if (d > polygon_resolution) // if distance is too far, interpolate
				{
		...
		// Polygon may be changed. Update center of polygon
	...
	std::vector<QuarterView> PolygonGenerator::CreateQuarterViews(const int &nResolution)
	{
		...
		{
			QuarterView q(angle, angle + range, i); // (min, max, index)
