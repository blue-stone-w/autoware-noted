...
ContourTracker::~ContourTracker()
{
  if (m_Params.bEnableLogging == true)
  {
    ...
    UtilityHNS::DataRW::WriteLogData(
        UtilityHNS::UtilityH::GetHomeDirectory() + UtilityHNS::DataRW::LoggingMainfolderName + UtilityHNS::DataRW::TrackingFolderName,
        "contour_tracker",
...
void ContourTracker::ReadNodeParams()
{
  ...
  // suggestion: replace "if" with "switch"
  if (tracking_type == 0)
...
void ContourTracker::ImportCloudClusters(const autoware_msgs::CloudClusterArrayConstPtr &msg,
                                         std::vector<PlannerHNS::DetectedObject> &originalClusters)
{
  ...
  m_FilteringTime      = 0;
  m_PolyEstimationTime = 0;
  struct timespec filter_time, poly_est_time; // time to judge whethen it locates on lanes(in map);
                                              // time to generate poly for one cloud cluster
  ...
  } // endfor: have traversed all clusters
}
                           ...
                           PlannerHNS::RoadNetwork &map)
{
  if (bMap)
  {
    ...
    for (unsigned int i = 0; i < m_ClosestLanesList.size(); i++)
    {
      ...
      //	std::cout << "- Distance To Car: " << obj.distance_to_center << ", PerpD: " << info.perp_distance
      //<< ", DirectD: " << direct_d << ", bAfter: " << info.bAfter << ", bBefore: " << info.bBefore << std::endl;
  ...
  if (!m_Params.bEnableSimulation)
  {
    ...
    if (obj.distance_to_center > m_Params.DetectionRadius
        || object_size < m_Params.MinObjSize
        || object_size > m_Params.MaxObjSize)
      return false;
  }
  ...
  if (m_Params.bEnableSimulation)
  {
    ...
    double distance_x = fabs(relative_point.x - m_Params.VehicleLength / 3.0); // ???why 3.0
...
void ContourTracker::LogAndSend()
{
  ...
  dataLine << UtilityHNS::UtilityH::GetLongTime(log_t) << "," << m_dt
           ...
           << m_ObstacleTracking.m_DetectedObjects.size() - m_OriginalClusters.size() << "," << m_nOriginalPoints
           << "," << m_nContourPoints << "," << m_FilteringTime << "," << m_PolyEstimationTime
           << "," << m_tracking_time << "," << m_tracking_time + m_FilteringTime + m_PolyEstimationTime << ",";
...
void ContourTracker::GetFrontTrajectories(std::vector<PlannerHNS::Lane *> &lanes,
